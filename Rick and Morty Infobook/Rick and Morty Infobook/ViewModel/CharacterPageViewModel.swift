...
class CharacterPageViewModel: ObservableObject {
    ...
    @Published private(set) var page: CharacterPage? = nil
    var pageNum: Int = 1
    ...
    func load() {
        guard let url = URL(string: "https://rickandmortyapi.com/api/character?page=\(pageNum)") else {
        ...
        URLSession.shared.dataTask(with: request) {(data,response,error) in
            do {
                if let d = data {
                    ...
                    let decodedLists = try decoder.decode(CharacterPage.self, from: d)
