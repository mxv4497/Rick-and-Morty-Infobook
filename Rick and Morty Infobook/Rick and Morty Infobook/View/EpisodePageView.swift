...
struct EpisodePageView: View {
    @StateObject private var viewModel = EpisodePageViewModel()
            ...
            VStack{
                HStack(spacing: 8){
                    VStack{
                        ...
                        Text("\(viewModel.page?.info.count ?? 0) episodes")
                            .font(.callout)
                            .padding(.top)
                    }
                ...
                ScrollView{
                    LazyVStack{
                        ForEach(viewModel.page?.results ?? []) { episode in
...
struct EpisodeRow: View{
    ...
    var body: some View {
        HStack{
            ...
            Spacer()
        }.background(Color.lime.opacity(0.05))
    }
}
